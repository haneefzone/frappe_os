...
Rechained onto the 5.2 panel-copilot head (a1d2c3b4e5f6) when DOO-166 and
DOO-167 merged in the same window, to keep a single alembic head. Idempotent:
`upgrade()` creates the three tables + indexes, `downgrade()` drops them
cleanly (up/down/up verified).

Revision ID: f7a2c9e1b3d5
Revises: a1d2c3b4e5f6
...
down_revision: str | Sequence[str] | None = 'a1d2c3b4e5f6'
