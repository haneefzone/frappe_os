...
- [ ] **1.4 Live logs.** Worker persists LogEntry batches + publishes to pub/sub; SSE endpoint with replay + tail + heartbeat; Jobs page (live table) + Job detail (step timeline left, live LogViewer right, sanitized params, "Show exact command" expander, role-gated Cancel/Retry); persistent bottom-right job tray.
